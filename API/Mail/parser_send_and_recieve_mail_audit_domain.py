...
- Статистика по доменным зонам (.ru/.com/.cloud и т.д.)
'''
...
import gc
import traceback
from email.utils import getaddresses  # ДОБАВЛЕНО
...
PERIOD_DAYS = 31  # используется если DATE_FROM/DATE_TO = None
...
# ----------------------------------------------------------------------------
# "tld"    -> .ru/.com/.cloud
# "domain" -> полный домен: gmail.com/yandex.ru (если понадобится)
DOMAIN_STAT_MODE = "tld"
TOP_DOMAINS_IN_REPORT = 50
...
class MailStatsCollector:
    ...
    def __init__(
        ...
        self.stats = {
            ...
            # ДОБАВЛЕНО
            'domain_sent': defaultdict(int),      # отправлено НА доменную зону
            'domain_received': defaultdict(int),  # получено С доменной зоны
        }
    ...
    def _normalize_domain(self, domain: str) -> str:
        ...
        if DOMAIN_STAT_MODE == "domain":
            return domain

        # режим "tld" -> .ru/.com/.cloud
        parts = domain.split(".")
        if len(parts) < 2:
            return "unknown"
        return "." + parts[-1]

    def _extract_domains(self, header_value: str) -> List[str]:
        """
        Достаёт доменные зоны из to/cc/bcc/from.
        Корректно парсит строки вида: 'Name <a@b.com>, c@d.ru'
        """
        if not header_value:
            return []
        domains: List[str] = []
        for _, email in getaddresses([header_value]):
            ...
            domains.append(self._normalize_domain(domain))
        return domains
    # ------------------------------------------------------------------------
    ...
    def _process_single_event(self, event: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            with self._stats_lock:
                ...
                # ДОБАВЛЕНО: статистика по доменным зонам
                if is_receive:
                    from_domains = self._extract_domains(event.get('from', ''))
                    dom = from_domains[0] if from_domains else "unknown"
                    self.stats['domain_received'][dom] += 1

                elif is_send:
                    to_domains = self._extract_domains(event.get('to', ''))
                    cc_domains = self._extract_domains(event.get('cc', ''))
                    bcc_domains = self._extract_domains(event.get('bcc', ''))

                    unique_recipient_domains = set(to_domains + cc_domains + bcc_domains)
                    if not unique_recipient_domains:
                        unique_recipient_domains = {"unknown"}

                    # 1 письмо учитываем по каждому домену 1 раз (уникальные домены письма)
                    for dom in unique_recipient_domains:
                        self.stats['domain_sent'][dom] += 1
    ...
    def _save_checkpoint(self):
        ...
        checkpoint = {
            ...
            'stats': {
                ...
                # ДОБАВЛЕНО (краткая справка)
                'domain_sent_keys': len(self.stats['domain_sent']),
                'domain_received_keys': len(self.stats['domain_received']),
            }
        }
        try:
    ...
    # ------------------------------------------------------------------------
    # CSV со статистикой по доменным зонам
    # ------------------------------------------------------------------------
    def _write_domain_stats_csv(self):
        """Записывает статистику по доменным зонам (получено/отправлено)."""
        csv_path = os.path.join(self.output_dir, "domain_statistics.csv")

        all_domains = set(self.stats['domain_sent'].keys()) | set(self.stats['domain_received'].keys())

        with open(csv_path, 'w', newline='', encoding='utf-8-sig') as f:
            writer = csv.DictWriter(
                f,
                fieldnames=['domain_zone', 'sent_messages', 'received_messages', 'total_messages'],
                delimiter=';',
            ...
            for zone in sorted(
                all_domains,
                key=lambda d: (self.stats['domain_sent'].get(d, 0) + self.stats['domain_received'].get(d, 0)),
                reverse=True
            ):
                s = self.stats['domain_sent'].get(zone, 0)
                r = self.stats['domain_received'].get(zone, 0)
                writer.writerow({
                    'domain_zone': zone,
                    'sent_messages': s,
                    'received_messages': r,
        ...
        self.logger.info(f"Статистика по доменным зонам: {csv_path}")
    # ------------------------------------------------------------------------
    ...
    def _generate_report(self):
        ...
        # --------------------------------------------------------------------
        # СТАТИСТИКА ПО ДОМЕННЫМ ЗОНАМ (.ru/.com/...) — ТАБЛИЦЫ
        # --------------------------------------------------------------------
        
        lines.extend([
            "-" * 80,
            "СТАТИСТИКА ПО ДОМЕННЫМ ЗОНАМ (например .ru/.com/.cloud)",
            "-" * 80,
            "",
            "-" * 80,
            "ПОЛУЧЕНО СООБЩЕНИЙ ПО ДОМЕННЫМ ЗОНАМ (FROM)",
            "-" * 80,
            f"{'Домен':<12} | {'Получено (FROM)':>16}",
            "-" * 80,
        ])

        top_recv = sorted(
            self.stats['domain_received'].items(),
            key=lambda x: x[1],
            reverse=True
        )[:TOP_DOMAINS_IN_REPORT]

        if top_recv:
            for zone, cnt in top_recv:
                lines.append(f"{zone:<12} | {cnt:>16,}")
        else:
            lines.append("  (нет данных)")
        ...
        lines.extend([
            ...
            "",
            "-" * 80,
            "ОТПРАВЛЕНО СООБЩЕНИЙ ПО ДОМЕННЫМ ЗОНАМ (TO/CC/BCC)",
            "-" * 80,
            f"{'Домен':<12} | {'Отправлено (TO/CC/BCC)':>24}",
            "-" * 80,
        ])

        top_sent = sorted(
            self.stats['domain_sent'].items(),
            key=lambda x: x[1],
            reverse=True
        )[:TOP_DOMAINS_IN_REPORT]

        if top_sent:
            for zone, cnt in top_sent:
                lines.append(f"{zone:<12} | {cnt:>24,}")
        else:
            lines.append("  (нет данных)")
        ...
        lines.extend([
            ...
            "  domain_statistics.csv   - статистика по доменным зонам (.ru/.com/...)",
...
def main():
    ...
    print(f"✓ Доменная статистика: {('TLD (.ru/.com)' if DOMAIN_STAT_MODE == 'tld' else 'FULL DOMAIN')}")
    print("=" * 60)
