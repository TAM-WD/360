### Скрипт v3 — выгрузка пользователей организации Яндекс 360
# Используется API Яндекс 360 для бизнеса
# Поддержка: departments API, groups API, CSV/XLSX, интерактивное меню, отчёт
# Изменения v3:
#   - Добавлена колонка groupsName (названия групп)
#   - Загрузка групп через API (get-groups / get-group-info)
#   - Убрана колонка externalId
#   - Ввод токена показывает рандомные символы вместо скрытого ввода
...
# Символы для маскировки токена при вводе и отображении
MASK_SYMBOLS = "█▓▒░◆◇●○■□▲△◈◉★☆♦♠♣♥♤♧♡⬟⬡⬢⯃⯂"
...
# ── Группы ────────────────────────────────────────────────────────────────────
# Справка: https://yandex.ru/dev/api360/doc/ru/directory/get-group-info
#   GET /directory/v1/org/{orgId}/groups/{groupId}
# Справка: https://yandex.ru/dev/api360/doc/ru/directory/get-groups
#   GET /directory/v1/org/{orgId}/groups

def api360_get_groups(orgid, page=1, per_page=PERPAGE):
    """GET /directory/v1/org/{orgId}/groups — список всех групп организации."""
    url = f"{BASE_URL}/{orgid}/groups"
    params = {"page": page, "perPage": per_page}
    r = _session().get(url, params=params, headers=_headers())
    r.raise_for_status()
    data = r.json()
    return data.get("groups", []), data.get("pages", 1)


def api360_get_group_info(orgid, group_id):
    """GET /directory/v1/org/{orgId}/groups/{groupId} — информация о группе.

    Возвращает объект группы с полями:
      id, name, type, description, label, membersCount, email,
      aliases, removed, memberOf, createdAt
    """
    url = f"{BASE_URL}/{orgid}/groups/{group_id}"
    r = _session().get(url, headers=_headers())
    r.raise_for_status()
    return r.json()


def fetch_all_groups(orgid):
    """Загружает ВСЕ группы (с пагинацией) и возвращает dict {id: group}."""
    groups = {}
    page = 1
    total_pages = 1
    while page <= total_pages:
        grps, total_pages = api360_get_groups(orgid, page=page, per_page=PERPAGE)
        for g in grps:
            groups[g["id"]] = g
        page += 1
    return groups
...
def mask_token(token: str) -> str:
    """Маскирует токен рандомными символами для отображения."""
    if not token:
        return "(пусто)"
    masked = ""
    for _ in token:
        masked += random.choice(MASK_SYMBOLS)
    return masked


def read_token_visible():
    """Читает токен посимвольно, показывая рандомные символы вместо реальных.

    На Windows использует msvcrt для посимвольного чтения.
    На других ОС — fallback через getpass.
    """
    if os.name == "nt":
        try:
            import msvcrt
            sys.stdout.write("  Токен > ")
            sys.stdout.flush()
            token_chars = []
            while True:
                ch = msvcrt.getwch()
                if ch in ("\r", "\n"):
                    ...
                    if token_chars:
                        token_chars.pop()
                        # Стираем символ на экране: курсор назад, пробел, курсор назад
                        ...
                        if user32.OpenClipboard(0):
                            try:
                                ...
                                if h_data:
                                    ...
                                    if ptr:
                                        clipboard_text = ctypes.wstring_at(ptr)
                                        kernel32.GlobalUnlock(h_data)
                                        for paste_ch in clipboard_text:
                                            if paste_ch in ("\r", "\n"):
                                                continue
                                            token_chars.append(paste_ch)
                                            sys.stdout.write(random.choice(MASK_SYMBOLS))
                                            sys.stdout.flush()
                            finally:
                                user32.CloseClipboard()
                    except Exception:
                        # Если не удалось прочитать буфер — игнорируем
                        pass
                else:
                    token_chars.append(ch)
                    # Показываем рандомный символ вместо реального
                    sys.stdout.write(random.choice(MASK_SYMBOLS))
                    sys.stdout.flush()
    ...
    # Fallback для не-Windows или если msvcrt недоступен
    import getpass
    return getpass.getpass("  Токен > ")
...
def print_header():
    print("=" * 60)
    print("  Яндекс 360 — Выгрузка пользователей организации  v3.0")
    print("=" * 60)
...
def menu_set_token():
    global TOKEN
    print()
    print("  Введите OAuth-токен (символы маскируются при вводе):")
    token = read_token_visible()
    if token.strip():
        TOKEN = token.strip()
...
def flatten_user(user: dict, dept_map: dict, group_map: dict) -> dict:
    ...
    if isinstance(user_groups, list):
        for g in user_groups:
            ...
            if gid in group_map:
                group_names.append(group_map[gid].get("name", str(gid)))
            else:
...
def run_export():
    ...
    print("  [1/4] Загрузка подразделений...")
    try:
    ...
    # ── 2. Загрузка групп ─────────────────────────────────────────────────────
    print("  [2/4] Загрузка групп...")
    group_count = 0
    try:
        group_map = fetch_all_groups(ORGID)
        group_count = len(group_map)
        print(f"         Найдено групп: {group_count}")
    except requests.exceptions.HTTPError as e:
        print(f"  ✘ Ошибка при загрузке групп: {e}")
        print("    Возможно, у токена нет прав directory:read_groups.")
        print("    Продолжаю без названий групп...")
        group_map = {}
    except Exception as e:
        print(f"  ✘ Ошибка: {e}")
        group_map = {}
    ...
    print("  [3/4] Загрузка пользователей...")
    all_rows = []
    page = 1
    total_pages = 1
    user_count = 0
    ...
    try:
        while page <= total_pages:
            ...
            for user in users:
                flat = flatten_user(user, dept_map, group_map)
    ...
    print(f"  [4/4] Сохранение в {ext.upper()}...")
    ...
    print(f"  ║  Групп найдено          : {group_count:<26}║")
